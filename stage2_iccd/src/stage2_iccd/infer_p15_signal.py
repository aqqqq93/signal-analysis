...
def infer_signal(
    ...
    signal_np = (signal_np - signal_np.mean()) / max(float(signal_np.std()), 1.0e-6)
    ...
    pipeline = P15Stage2Pipeline(P15PipelineConfig(use_scenario_hints=scenario_hint is not None), device=device)
    out, route = pipeline.run(batch, fs, scenario_hints=[scenario_hint] if scenario_hint else None)

    out_dir = Path(output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    if_hz = out["identity_stable_if_hz"][0].detach().cpu().numpy()
    ...
    save_inference_plot(plot_path, signal_np, if_hz, fs, active_count=active_pred)
    payload = {
...
def save_inference_plot(path: Path, signal: np.ndarray, if_hz: np.ndarray, fs: float, active_count: int) -> None:
    ...
    ax.set_title("P1.5 pipeline inference")
